...
function DesignFileSection({ bid, project, users, setBids, setProject }) {
  ...
  const handleSubmit = async(e)=>{
        ...
        await axios.post(`${url}/designFile`,formData,{withCredentials:true}).then((resp)=>{
  ...
  const handlApprove = async(id)=>{
    ...
    await axios.post(`${url}/designFile/approve`,request,{withCredentials:true}).then((resp)=>{
        // console.log(resp.data);
        if(resp.data.error) return setOpenError({open:true,message:`${resp.data.error}`})
  ...
  return (
    ...
    {/* REPORT MODAL */}
      ...
      <ModalHeader>Upload Design File</ModalHeader>
      <ModalBody>
...
export default DesignFileSection;
