    ...
    function openModal(){
      ...
      useEffect(()=>{
        const getData=async()=>{
            ...
            await axios.get(`${url}/members/${id}`,{withCredentials:true}).then((resp)=>{
                // console.log(resp.data);
              if(resp.data.error){
                setOpenError({open:true,message:true})
              }else{
                setMembers(resp.data)
                setMembersForm({name:resp.data.Name,description:resp.data.description,position:resp.data.position,image:resp.data.position})
              }
            })
  ...
  const handleDelete = async()=>{
    ...
    return (
      <>
        ...
        <PageTitle>Member | {members.Name}</PageTitle>
        <ErrorAlert
        open={openError.open}
      ...
      {/* Delete Confirm section */}
        ...
        <Button onClick={openModal}>Update Member</Button>
  
      
        </TableContainer>

        <Modal isOpen={isOpen} onClose={closeModal}>
      <ModalHeader>Updaing Member {members.Name}</ModalHeader>
      <ModalBody>
      ...
      <Table>
        ...
        <TableBody>
            ...
            <Fragment>
              <TableRow>
                <TableCell><span className="text-sm font-semibold">{members?.Name}</span></TableCell>
                <TableCell><span className="text-sm font-semibold">{members?.position}</span></TableCell>
               
                <TableCell className="flex justify-center space-x-2">
                  <Button layout="link" size="small" onClick={() => openDelete(members.id)}>
