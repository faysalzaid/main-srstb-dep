...
function OrderFileSection({ bid, project, users, setBids, setProject }) {
  ...
  const {authState} = useContext(AuthContext)
    useEffect(()=>{
        const getData =async()=>{
          await axios.get(`${url}/orderFile/single/${project?.id}`,{withCredentials:true}).then((resp)=>{
  ...
  const handleSubmit = async(e)=>{
        ...
        await axios.post(`${url}/orderFile`,formData,{withCredentials:true}).then((resp)=>{
  ...
  const handlApprove = async(id)=>{
    ...
    await axios.post(`${url}/orderFile/approve`,request,{withCredentials:true}).then((resp)=>{
        // console.log(resp.data);
        if(resp.data.error) return setOpenError({open:true,message:`${resp.data.error}`})
        setOrderFileData(resp.data)
  ...
  const handleDelete = async(id)=>{
    ...
    // console.log('request is ',request);
    await axios.delete(`${url}/orderFile/${id}`,{withCredentials:true}).then((resp)=>{
        // console.log(resp.data);
        if(resp.data.error) return setOpenError({open:true,message:`${resp.data.error}`})
  ...
  return (
    ...
    {/* REPORT MODAL */}
      ...
      <ModalHeader>Upload Order File</ModalHeader>
      <ModalBody>
...
export default OrderFileSection;
